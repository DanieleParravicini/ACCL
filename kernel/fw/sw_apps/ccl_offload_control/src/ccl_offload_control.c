...
static volatile unsigned int
    dma_tag_lookup[MAX_DMA_TAGS];  // index of the spare buffer that has been
                                   // issued with that dma tag. -1 otherwise
static volatile unsigned int dma_segment_size = DMA_MAX_BTT;
...
static inline void enable_irq() {
  ...
  Xil_Out32(
      IRQCTRL_BASEADDR + IRQCTRL_MASTER_ENABLE_REGISTER_OFFSET,
      IRQCTRL_MASTER_ENABLE_REGISTER_HARDWARE_INTERRUPT_ENABLE |
          IRQCTRL_MASTER_ENABLE_REGISTER_MASTER_ENABLE);
}
...
static inline void ack_irq(int mask) {
  ...
  Xil_Out32(
      IRQCTRL_BASEADDR + IRQCTRL_INTERRUPT_ACKNOWLEDGE_REGISTER_OFFSET, mask);
}
...
static inline void start_timeout(unsigned int time) {
  // 1. set timer load register x(TLRx) with time so that TIMING_INTERVAL =
  // (TLRx + 2) * AXI_CLOCK_PERIOD
  // 2. load timer register
  // 3. set counter enable/down/non-autoreload/interrupt enable/clear load
  // /generate mode
  Xil_Out32(TIMER_BASEADDR + TIMER0_LOAD_REGISTER_OFFSET, time);
  Xil_Out32(
      TIMER_BASEADDR + TIMER0_CONTROL_AND_STATUS_REGISTER_OFFSET,
      CONTROL_AND_STATUS_REGISTER_LOAD_TIMER_MASK);
  Xil_Out32(
      TIMER_BASEADDR + TIMER0_CONTROL_AND_STATUS_REGISTER_OFFSET,
      CONTROL_AND_STATUS_REGISTER_ENABLE_MASK |
          CONTROL_AND_STATUS_REGISTER_INTERRUPT_ENABLE_MASK |
          CONTROL_AND_STATUS_REGISTER_UP_DOWN_MASK);
}
...
static inline void clear_timer_interrupt() {
  Xil_Out32(
      TIMER_BASEADDR + TIMER0_CONTROL_AND_STATUS_REGISTER_OFFSET,
      CONTROL_AND_STATUS_REGISTER_INTERRUPT_MASK);
}
...
static inline void start_timer1() {
  ...
  Xil_Out32(
      TIMER_BASEADDR + TIMER1_CONTROL_AND_STATUS_REGISTER_OFFSET,
      CONTROL_AND_STATUS_REGISTER_LOAD_TIMER_MASK);
  Xil_Out32(
      TIMER_BASEADDR + TIMER1_CONTROL_AND_STATUS_REGISTER_OFFSET,
      CONTROL_AND_STATUS_REGISTER_ENABLE_MASK);
...
// for hls ip protocols
// https://www.xilinx.com/html_docs/xilinx2020_2/vitis_doc/managing_interface_synthesis.html#qls1539734256651__ae476333
void start_arith(
    unsigned int byte_count,
    unsigned int function,
    unsigned use_repeat) {
  // get number of 512b/64B transfers corresponding to byte_count
  unsigned int num_dma_transfers =
      (unsigned int)(byte_count / dma_segment_size);
  unsigned int bytes_tail = byte_count % dma_segment_size;
  unsigned int quad_word_per_dma_transfer =
      (unsigned int)((dma_segment_size + 63) / 64);
  unsigned int quad_word_per_tail = (unsigned int)((bytes_tail + 63) / 64);
  unsigned int quad_word_stream_transfers =
      num_dma_transfers * quad_word_per_dma_transfer + quad_word_per_tail;
  Xil_Out32(ARITH_BASEADDR + 0x10, quad_word_stream_transfers);
  Xil_Out32(ARITH_BASEADDR + 0x18, function);
  SET(ARITH_BASEADDR,
      CONTROL_START_MASK | (use_repeat == 1 ? CONTROL_REPEAT_MASK : 0));
}
...
static inline void wait_arith() {
  ...
  for (unsigned int count = 0; !arith_is_idle(); count++) {
    if (timeout != 0 && count >= timeout) longjmp(excp_handler, ARITH_ERROR);
  }
}
...
// issue a dma command
static inline void dma_cmd_addrh_addrl(
...
static inline void dma_cmd_without_EOF(
    unsigned int channel,
    unsigned int btt,
    uint64_t addr,
    unsigned int tag) {
...
static inline void
set_switch_datapath(unsigned int base, unsigned int src, unsigned int dst) {
...
// PG085 page 26
// config registers start at 0x40
static inline void disable_switch_datapath(
    unsigned int base,
    unsigned int dst) {
...
//Configure the arith switch to select either the external or the internal ALU
// redirecting operands and result from/to the external arith or the internal ALU
//" "          "       "
// ARITH_NONE leaves the current arithmetic switch configuration
void cfg_arith_switch(unsigned int arith ){
  switch (arith) {
    case ARITH_EXTERNAL:
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_OP0, ARITH_SWITCH_M_EXT_OP0);
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_OP1, ARITH_SWITCH_M_EXT_OP1);
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_EXT_RES, ARITH_SWITCH_M_RES);
    ...
    case ARITH_INTERNAL:
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_OP0, ARITH_SWITCH_M_INT_OP0);
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_OP1, ARITH_SWITCH_M_INT_OP1);
      set_switch_datapath(
          ARITH_SWITCH_BASEADDR, ARITH_SWITCH_S_INT_RES, ARITH_SWITCH_M_RES);
...
//   							   DMA1 RX -> Arith Op1
//   							   Arith Res -> DMA1 TX
// 3 == DATAPATH_OFFCHIP_TX_UDP   : DMA1 RX -> UDP TX (UDP packetizer) (Direct
// Off-chip TX) 4 == DATAPATH_OFFCHIP_TX_TCP   : DMA1 RX -> TCP TX (TCP
// packetizer)    (Direct Off-chip TX) 5 == DATAPATH_OFFCHIP_UDP_REDUCTION:
// DMA0 RX -> Arith Op0 (Reduced Off-chip TX)
//   									DMA1 RX ->
//   Arith
//   Op1 									Arith Res -> UDP TX(UDP
//   packetizer)
// 6 == DATAPATH_OFFCHIP_TCP_REDUCTION: 	DMA0 RX -> Arith Op0 (Reduced
// Off-chip TX)
//   									DMA1 RX ->
//   Arith
//   Op1 									Arith Res -> TCP TX (TCP
//   packetizer)
// 7 == DATAPATH_DMA_EXT_LOOPBACK:		DMA0_RX -> M_EXT_KRNL (TX)
//										S_EXT_KRNL (RX)
//-> DMA1_TX there's a subswitch that is specific for reduce:
// you can select where to redirect operands to internal or external arithmetic
// unit
//									result from
//" "          "       "
// ARITH_NONE leaves the current arithmetic switch configuration
...
static inline void start_packetizer(
    uint64_t base_addr,
...
// Since at this level we are not sure the other ccl_offlaod instances have open
// their port this function is not working.
...
static inline unsigned int segment(
    unsigned int number_of_bytes,
    unsigned int segment_size) {
  return (number_of_bytes + segment_size - 1) / segment_size;
}

void static inline start_dma(
    unsigned int len,
  ...
  //to communicate with dma_mover
  //you put on MB CMD_DMA_MOVER the following arguments
  // len 		    32 bits
  // dst_rank 	32 bits	
  // mpi_tag 	  32 bits
  // op0_addr 	32 bits	
...
void static inline start_dma_and_packetizer(
  communicator *world,
    unsigned int len,
    uint64_t DMA0_rx_addr,
    uint64_t DMA1_rx_addr,
...
static inline int ack_dma_and_packetizer(){
  return ( getd( STS_DMA_MOVER) == 0 ? COLLECTIVE_OP_SUCCESS : DMA_INTERNAL_ERROR);
}

static inline int ack_dma(){
  return ack_dma_and_packetizer();
}
...
static inline int dma_movement(
    ...
    uint64_t DMA2_rx_addr,
    unsigned int what_DMAS) {
    start_dma(len, DMA0_rx_addr, DMA1_rx_addr, DMA1_tx_addr, DMA2_rx_addr, what_DMAS);
    return ack_dma();
...
static inline int dma_movement_and_packetizer(
    ...
    unsigned dst_rank,
    unsigned mpi_tag) {
    start_dma_and_packetizer(world, len, DMA0_rx_addr, DMA1_rx_addr, DMA1_tx_addr, DMA2_rx_addr, what_DMAS, dst_rank, mpi_tag);
    return ack_dma_and_packetizer();
}

static inline int dma_movement_single(
    unsigned int len,
    uint64_t DMA0_rx_addr,
    uint64_t DMA1_rx_addr,
    uint64_t DMA1_tx_addr,
  ...
  return dma_movement(len, DMA0_rx_addr, DMA1_rx_addr, DMA1_tx_addr, DMA2_rx_addr, what_DMAS);
}

// performs a copy using DMA1. DMA1 rx reads while DMA1 tx overwrites
static inline int copy(unsigned int len, uint64_t src_addr, uint64_t dst_addr) {
  // configure switch
  cfg_switch(DATAPATH_DMA_LOOPBACK, ARITH_NONE);
  // call function that will control the DMAs
  return dma_movement(len, 0, src_addr, dst_addr, 0, USE_OP1_DMA | USE_RES_DMA);
}

// performs an accumulate using DMA1 and DMA0. DMA0 rx reads op1 DMA1 rx reads
// op2 while DMA1 tx overwrites op2 buffer
static inline int accumulate(
  ...
  // start dma transfer
  unsigned int ret = dma_movement( len, op0_addr, op1_addr, dst_addr, 0, USE_OP0_DMA | USE_OP1_DMA | USE_RES_DMA);
...
int transmit_offchip_world(
  ...
  return dma_movement_and_packetizer( world, len, 0, src_addr, 0, 0, USE_OP1_DMA, dst_rank, dst_tag);
}
...
// performs an accumulate using DMA0 and DMA1 and internal arith unit. Then it
// forwards the result through tx_subsystem to dst_rank
int reduce_offchip_world(
  ...
  // configure switch
  cfg_switch(
      use_tcp ? DATAPATH_OFFCHIP_TCP_REDUCTION : DATAPATH_OFFCHIP_UDP_REDUCTION,
      ARITH_INTERNAL);
  // configure arith
  start_arith(len, func, 0);
  return dma_movement_and_packetizer( world, len, op0_addr, op1_addr, 0, 0, USE_OP0_DMA | USE_OP1_DMA, dst_rank, dst_tag);
}
...
int t_wait_receive_world_i(
  ...
  unsigned int seq_num;  // src_port TODO: use this variable to choose depending
                         // on tcp/udp session id or port
  int i;
  seq_num = world->ranks[src_rank].inbound_seq + 1;
  // TODO: use a list to store recent message received to avoid scanning in the
  // entire spare_buffer_struct. parse rx buffers until match is found matches
  // len, src matches tag or tag is ANY return buffer index
  unsigned int nbufs = Xil_In32(RX_BUFFER_COUNT_OFFSET);
  rx_buffer *rx_buf_list = (rx_buffer *)(RX_BUFFER_COUNT_OFFSET + 4);
  for (i = 0; i < nbufs; i++) {
    microblaze_disable_interrupts();
    if (rx_buf_list[i].status == STATUS_RESERVED) {
      if ((rx_buf_list[i].rx_src == src_rank) &&
          (rx_buf_list[i].rx_len == len)) {
        if (((rx_buf_list[i].rx_tag == src_tag) || (src_tag == TAG_ANY)) &&
            (rx_buf_list[i].sequence_number == seq_num)) {
...
int receive_offchip_world(
  ...
  unsigned int remaining_to_move, remaining_to_ack, dma_tag_tmp, curr_len_move,
      curr_len_ack, i;
  rx_buffer *rx_buf_list = (rx_buffer *)(RX_BUFFER_COUNT_OFFSET + 4);
  ...
  for (i = 0; remaining_to_move > 0 && i < max_dma_in_flight; i++) {
    ...
    // start DMAs
    start_dma(curr_len_move, 0, rx_buff_addr, dst_addr, 0, USE_OP1_DMA | USE_RES_DMA);
  ...
  while (remaining_to_move > 0) {
    if (remaining_to_ack < dma_segment_size)
      curr_len_ack = remaining_to_ack;
    // wait for DMAs to finish
    ack_dma(curr_len_ack, USE_OP1_DMA | USE_RES_DMA);
...
int receive_and_accumulate(
  ...
  for (i = 0; remaining_to_move > 0 && i < max_dma_in_flight; i++) {
    ...
    spare_buffer_indexes_end = (spare_buffer_indexes_end + 1) % max_dma_in_flight;
    // start DMAs
    start_dma(curr_len_move, op0_addr, rx_buff_addr, dst_addr, 0, USE_OP0_DMA | USE_OP1_DMA | USE_RES_DMA);
    remaining_to_move -= curr_len_move;
  ...
  while (remaining_to_move > 0) {
    if (remaining_to_ack < dma_segment_size)
      curr_len_ack = remaining_to_ack;
    // wait for DMAs to finish
    ack_dma(curr_len_ack, USE_OP0_DMA | USE_OP1_DMA | USE_RES_DMA);
    // set spare buffer as free
    buf_idx = spare_buffer_indexes[spare_buffer_indexes_start];
    spare_buffer_indexes_start  = (spare_buffer_indexes_start + 1) % max_dma_in_flight;
    rx_buf_list[buf_idx].status = STATUS_IDLE;
...
int receive_and_reduce_offchip(
  ...
  for (i = 0; remaining_to_move > 0 && i < max_dma_in_flight; i++) {
    ...
    spare_buffer_indexes_end = (spare_buffer_indexes_end + 1) % max_dma_in_flight;
    // start pack and DMAs
    start_dma_and_packetizer( world, curr_len_move, op0_addr, rx_buff_addr, 0, 0, USE_OP0_DMA | USE_OP1_DMA, dst_rank, mpi_tag);
    remaining_to_move -= curr_len_move;
    op0_addr          += curr_len_move;
  }
  // 2.ack 1 and issue another dma transfer up until there's no more dma move to
...
int relay_to_other_rank(
  ...
  for (i = 0; remaining_to_move > 0 && i < max_dma_in_flight; i++) {
    ...
    spare_buffer_indexes_end = (spare_buffer_indexes_end + 1) % max_dma_in_flight;
    // start DMAs
    start_dma_and_packetizer( world, curr_len_move, 0, rx_buff_addr, 0, 0, USE_OP1_DMA, dst_rank, mpi_tag);
    remaining_to_move -= curr_len_move;
  }
  // 2.ack 1 and issue another dma transfer up until there's no more dma move to
  // issue
  while (remaining_to_move > 0) {
    if (remaining_to_ack < dma_segment_size)
      curr_len_ack = remaining_to_ack;
    // wait for DMAs and ACK to finish
    ack_dma_and_packetizer();
    // set spare buffer as free
    buf_idx = spare_buffer_indexes[spare_buffer_indexes_start];
    spare_buffer_indexes_start = (spare_buffer_indexes_start + 1) % max_dma_in_flight;
    ...
    buf_idx = wait_receive_world_i(world, src_rank, curr_len_move, mpi_tag);
    if (buf_idx < 0) return RECEIVE_OFFCHIP_SPARE_BUFF_ID_NOT_VALID;
    rx_buff_addr = ((uint64_t)rx_buf_list[buf_idx].addrh << 32) |
                   rx_buf_list[buf_idx].addrl;
    spare_buffer_indexes[spare_buffer_indexes_end] = buf_idx;
    spare_buffer_indexes_end = (spare_buffer_indexes_end + 1) % max_dma_in_flight;
    // start DMAs
    start_dma_and_packetizer( world, curr_len_move, 0, rx_buff_addr, 0, 0, USE_OP1_DMA, dst_rank, mpi_tag);
...
// root read multiple times the same segment and send it to each rank before
...
int broadcast_round_robin(
  ...
  if (src_rank == world.local_rank) {
    ...
    // send to all members of the communicator 1 segment of the buffer at the
    // time
    curr_len = dma_segment_size;
    while (len > 0) {
      if (len < dma_segment_size) curr_len = len;
      for (i = 0; i < world.size; i++) {
        if (i == world.local_rank) continue;
        // start packetizer messages and issue a single dma command for eachrank
        dma_movement_and_packetizer(&world,curr_len, 0, buf_addr, 0, 0, USE_OP1_DMA, i , TAG_ANY);
      }
...
// scatter segment at a time. root sends each rank a segment in a round robin
// fashion
int scatter_rr(
    unsigned int comm,
    unsigned int len,
  ...
  if (src_rank == world.local_rank) {
    ...
    // send to all members of the communicator 1 segment of the buffer at the
    // time w.r.t. broadcast the part that has to be sent is to i-esim_rank is
    // [i*len+:len]
    curr_len = dma_segment_size;
    while (len > 0) {
      if (len < dma_segment_size) curr_len = len;
      ...
      for (i = 0; i < world.size; i++) {
        if (i != world.local_rank) {
          // start packetizer messages and issue a single dma command for
          // eachrank
          start_dma_and_packetizer(&world, curr_len, 0, tmp_src_buf_offset_addr, 0, 0, USE_OP1_DMA, i, TAG_ANY);
        }
...
// naive gather: non root relay data to the root. root copy segments in dst
// buffer as they come.
int gather_ring(
    unsigned int comm,
    unsigned int len,
  ...
  if (root_rank == world.local_rank) {  // root ranks mainly receives
    ...
    for (i = 0, curr_pos = prev_in_ring;
         i < world.size && ret == COLLECTIVE_OP_SUCCESS;
         i++, curr_pos = (curr_pos + world.size - 1) % world.size) {
      // TODO: optimize receives; what we want is to move any data which arrives
      // into the correct segment of the buffer currently this will go
      // sequentially through the segments, which is slow and also requires more
      // spare RX buffers
      tmp_buf_addr = dst_buf_addr + len * curr_pos;
      if (curr_pos == world.local_rank) {  // root copies
        ret = copy(len, src_buf_addr, tmp_buf_addr);
      } else {
        ret = receive_offchip_world(
            &world, prev_in_ring, len, tmp_buf_addr, TAG_ANY);
      }
    }
  } else {
    // non root ranks sends their data + relay others data to the next rank in
    // sequence
    // as a daisy chain
    number_of_shift =
        ((world.size + world.local_rank - root_rank) % world.size) -
        1;  // distance to the root
    ret += transmit_offchip_world(
        &world, next_in_ring, len, src_buf_addr, TAG_ANY);
...
// naive fused: 1) receive a segment 2) move in the dest buffer 3) relay to next
// rank
int allgather_ring(
    unsigned int comm,
    unsigned int len,
  ...
  ret +=
      transmit_offchip_world(&world, next_in_ring, len, src_buf_addr, TAG_ANY);
  // receive from all members of the communicator
  for (i = 0, curr_pos = world.local_rank;
       i < world.size && ret == COLLECTIVE_OP_SUCCESS;
       i++, curr_pos = (curr_pos + world.size - 1) % world.size) {
    tmp_buf_addr = dst_buf_addr + len * curr_pos;
    ...
    if (curr_pos == world.local_rank) {
      ...
      ret = receive_offchip_world(
          &world, prev_in_ring, len, tmp_buf_addr, TAG_ANY);
      // TODO: use the same stream to move data both to dst_buf_addr and
      // tx_subsystem todo: use DMA1 RX to forward to next and DMA0/2 RX and
      // DMA1 TX to copy ~ at the same time!
      if (i + 1 < world.size) {  // if not the last data needed relay to the
                                 // next in the sequence
        ret = transmit_offchip_world(
            &world, next_in_ring, len, tmp_buf_addr, TAG_ANY);
      }
    }
  }
...
// every rank receives a buffer it sums its own buffer and forwards to next rank
// in the ring
int reduce_ring_streaming(
  ...
  if (prev_in_ring == root_rank) {
    ...
    // non root ranks sends their data + data received from previous rank to the
    // next rank in sequence as a daisy chain
    ret = receive_and_reduce_offchip(
        &world, prev_in_ring, next_in_ring, len, function, src_addr, TAG_ANY);
  } else {
    // root only receive from previous node tin the ring, add its local buffer
    // and save in destination buffer
    ret = receive_and_accumulate(
        &world, prev_in_ring, len, function, src_addr, dst_addr, TAG_ANY);
  }
  return ret;
}
...
int scatter_reduce(
  ...
  // 2. for n-1 times sum the chunk that is coming from previous rank with your
  // chunk at the same location. then forward the result to
  ...
  for (i = 0; i < world.size - 2;  ++i, curr_recv_chunk = (curr_recv_chunk + world.size - 1) % world.size) {
    ...
    // 2. receive part of data from previous in rank and accumulate to the part
    // you have at the same address ad forwar dto the next rank in the ring
    ret = receive_and_reduce_offchip( &world, prev_in_ring, next_in_ring, len, function, curr_recv_addr, TAG_ANY);
    if (ret != COLLECTIVE_OP_SUCCESS) return ret;
  }
  // at last iteration (n-1) you can sum and save the result at the same
  // location (which is the right place to be)
  if (world.size > 1) {
    curr_recv_addr = src_addr + len * curr_recv_chunk;
...
// 2 stage allreduce: distribute sums across the ranks. smaller bandwidth
// between ranks and use multiple arith at the same time.
// scatter_reduce+all_gather
int all_reduce_share(
  ...
  len_div_size +=  -tail + (tail == 0 ? 0 : min_operand_width); 
  // TODO: now when processing last chunk it
  // will assume that buffers are same length
  // and part of non buffer will be processed.
  unsigned int next_in_ring = (world.local_rank + 1) % world.size;
  unsigned int prev_in_ring = (world.local_rank + world.size - 1) % world.size;
  // scatter reduce
  ...
  for (i = 0; i < world.size - 1; ++i) {
    ...
    // TODO: 5.6. can be done together since we have 3 dmas (1rx read from
    // spare, 1tx write in the final dest, 2rx can read and transmit to next in
    // the ring)
    ...
    if (ret != COLLECTIVE_OP_SUCCESS) return ret;
    // 6. receive the reduced results from previous rank and put into correct
    // dst buffer position
...
int ext_kernel_stream(unsigned int len, uint64_t src_addr, uint64_t dst_addr) {
  // configure switch
...
int reduce_ext(
  ...
  return dma_movement(
      len, op1_addr, op2_addr, dst_addr, 0,
      USE_OP0_DMA | USE_OP1_DMA | USE_RES_DMA);
}
...
static inline unsigned int wait_for_call(void) {
  ...
  while (1) {
    if (tngetd(CMD_HOST) == 0) return CMD_HOST;
    if (tngetd(CMD_HLS ) == 0) return CMD_HLS;
  }
}
...
int main() {
  ...
  unsigned int buf0_addrl, buf0_addrh, buf1_addrl, buf1_addrh, buf2_addrl,
      buf2_addrh;
  uint64_t buf0_addr, buf1_addr, buf2_addr;

  init();
  // register exception handler though setjmp. it will save stack status to
  // reset stack when the jmp is performed 	ref:
  // https://www.gnu.org/software/libc/manual/html_node/Longjmp-in-Handler.html
  // when first executed, setjmp returns 0. call to longjmp pass a value != 0 to
  // indicate a jmp is occurred
