...
int main(){
    int nerrors  = 0;
    //1 dma transfer 
    nerrors 	+= test_dmas(USE_OP0_DMA				, 10);
    nerrors 	+= test_dmas(USE_OP1_DMA				, 10);
    nerrors 	+= test_dmas(USE_RES_DMA_WITHOUT_TLAST	, 10);
    nerrors 	+= test_dmas(USE_RES_DMA				, 10);
    cout << "Errors: " << nerrors  << " single dma transfer" << endl;
    //erroneous size
    nerrors 	 = test_dmas(USE_OP0_DMA				, 1);
    nerrors 	+= test_dmas(USE_OP1_DMA				, 2);
    nerrors 	+= test_dmas(USE_RES_DMA_WITHOUT_TLAST	, 3);
    nerrors 	+= test_dmas(USE_RES_DMA				, 7);
    cout << "Errors: " << nerrors  << "less than a single dma transfer" << endl;
    //multiple segments
    nerrors = 0;
    for(int i = 0; i < 10; i++){
    ...
    nerrors 	 = test_dmas(USE_OP0_DMA | USE_OP1_DMA | USE_RES_DMA_WITHOUT_TLAST, 100);
    cout << "Errors: " << nerrors  << " multiple dmas" << endl;
    nerrors 	 = test_dmas(USE_OP0_DMA | USE_OP1_DMA , 50);
    cout << "Errors: " << nerrors  << " multiple dmas" << endl;
