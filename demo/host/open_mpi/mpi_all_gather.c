...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < num_trials+1; i++) {
    ...
    starttime = MPI_Wtime();
    MPI_Allgather(data, send_count, MPI_INT, result, recv_count, MPI_INT, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    endtime = MPI_Wtime();
    duration = endtime - starttime;
    if(i>=1)
    {
      total_mpi_Allgather_time = total_mpi_Allgather_time + duration;
    }
  ...
  if (world_rank == 0) {
    printf("Allgather per rank send element count:%d, byte count:%ld, trials:%d, ", send_count, send_count*sizeof(int), num_trials);
    printf("Avg MPI_Allgather time [us]= *%lf\n", total_mpi_Allgather_time*1000000.0 / num_trials);
  }
