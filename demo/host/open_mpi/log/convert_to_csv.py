...
if __name__ == "__main__":
    ...
    for filename in tqdm(list(log_content)):
        ...
        if len(filename_parts) == 1 :
            ...
            with open(join(dst_base_path, f"mpi_{collective_name}.csv"), "a+", newline="") as f_out:
        ...
        filename_parts = re.findall("mpi_4_1_size_(\d+)_n_(\d+)_(\w+).txt", filename)
        if len(filename_parts) == 1 :
            #OpenMPI H2H 4_1
            experiment      = "converted from log"
            board_instance  = "OpenMPI 4_1"
            number_of_nodes = filename_parts[0][1]
            rank_id         = 0
            number_of_banks = 0
            buffer_size     = filename_parts[0][0] 
        ...
        filename_parts = re.findall("mpi_4_1_F2F_size_(\d+)_n_(\d+)_(\w+).txt", filename)
        if len(filename_parts) == 1 :
            #OpenMPI H2H 4_1 F2F
            experiment      = "converted from log"
            board_instance  = "OpenMPI 4_1 F2F"
            number_of_nodes = filename_parts[0][1]
            rank_id         = 0
            number_of_banks = 0
            buffer_size     = filename_parts[0][0] 
            ...
            with open(join(dst_base_path, f"mpi_{collective_name}.csv"), "a+", newline="") as f_out:
                ...
                for line in open(filename, "r").readlines():
                    ...
                    if collective_name == "Send/recv":
                        throughput  = float(buffer_size) *1024*8/(execution_time*1000)
                    csv_writer.writerow([experiment, board_instance, number_of_nodes, rank_id, number_of_banks, buffer_size, segment_size, collective_name, execution_time, throughput, execution_time_fullpath, throughput_fullpath])
        
        filename_parts = re.findall("mpi_4_1_F2F_H2H_size_(\d+)_n_(\d+)_(\w+).txt", filename)
        if len(filename_parts) == 1 :
            #OpenMPI H2H 4_1 F2F
            experiment      = "converted from log"
            board_instance  = "OpenMPI 4_1 new"
            number_of_nodes = filename_parts[0][1]
            rank_id         = 0
            number_of_banks = 0
            buffer_size     = filename_parts[0][0] 
            ...
            with open(join(dst_base_path, f"mpi_{collective_name}.csv"), "a+", newline="") as f_out:
                ...
                for line in open(filename, "r").readlines():
                    line_parts = re.findall("iteration:(\d+), F2F_time\[s\]:(\d+\.\d+), H2H_time:(\d+\.\d+)", line)
                    if len(line_parts) == 0 or len(line_parts) > 1:
                        continue

                    execution_time           = float(line_parts[0][1]) * pow(10,6)                
                    execution_time_fullpath  = float(line_parts[0][2]) * pow(10,6)  
                    if collective_name == "Send/recv":
                        throughput  = float(buffer_size) *1024*8/(execution_time*1000)
                        throughput_fullpath  = float(buffer_size) *1024*8/(execution_time_fullpath*1000)
                    csv_writer.writerow([experiment, board_instance, number_of_nodes, rank_id, number_of_banks, buffer_size, segment_size, collective_name, execution_time, throughput, execution_time_fullpath, throughput_fullpath])
        
        filename_parts = re.findall("mpi_4_1_mc_F2F_H2H_size_(\d+)_n_(\d+)_(\w+).txt", filename)
        if len(filename_parts) == 1 :
            #OpenMPI H2H 4_1 F2F
            experiment      = "converted from log"
            board_instance  = "OpenMPI 4_1 mc"
            number_of_nodes = filename_parts[0][1]
            rank_id         = 0
            number_of_banks = 0
            buffer_size     = filename_parts[0][0] 
